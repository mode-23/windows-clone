...
const Youtube = ({ user }) => {
  ...
  useEffect(() => {
    isLoading(true);
    fetchFromAPI(`trending?type=now`).then((data) => {
      setVideos(data.data);
      isLoading(false);
    });
  }, []);
  useEffect(() => {
    if (searchValue) {
      fetchFromAPI(`search?query=${searchValue}`).then((data) => {
  ...
  return (
    <div className={handleTabSize()}>
      <div
        className="tab_title youtube_tab_title"
            ...
            <div className="yt_search_drop_down">
              <ul>
                {refinements?.map((item, index) => (
                  <li
                    key={index}
                    onClick={() => navigate(`/youtube/search/${item}`)}
                  >
                    <NavLink to={`/youtube/search/${item}`} className="df">
                      <TfiSearch />
                      {item}
                    </NavLink>
                  </li>
          ...
          <div className="main">
            {links.map((item, index) => (
              <NavLink key={index} to={item.url} className="yt_nav_link" end>
                {item.icon}
                {expand ? item.shortName : item.name}
              </NavLink>
            ))}
          </div>
          {!expand && (
            <div className="explore">
              ...
              {sub_links.map((item, index) => (
                <NavLink
                  key={index}
                  to={item.url}
                  className="yt_nav_link yt_sub_nav_link"
                  end
                >
