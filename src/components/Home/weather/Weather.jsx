...
const Weather = ({ isOpenWeather }) => {
  ...
  return (
    <div className="tab weather">
      <div
        className="tab_title weather_title"
        onDoubleClick={() => isWideTab(!wideTab)}
      >
        <div className="df">
        ...
        <div className="tab_utils">
          ...
          <div
            className="tab_util_icon close"
            onClick={() => isOpenWeather(false)}
          >
            <IoCloseSharp />
          </div>
        </div>
        ...
        <>
          ...
          >
            ...
            <div className="df icon_wt">
              ...
              <h2>
                {data?.current_weather?.temperature?.toFixed(0)} <span>°C</span>
              </h2>
            </div>
          ...
          <motion.div
            className="weather_mid_menu_holder"
            ref={slideRef}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
              ...
              <div className="weather_mid_menu_daily">
                {data?.daily?.time?.map((item, index) => (
                  <div className="weather_mid_menu_daily_box" key={index}>
                    <p>
                      {days[new Date(item).getDay()]} {new Date(item).getDate()}
                    </p>
                    {renderCurrentImage(
            ...
            <motion.div className="weather_mid_menu weather_mid_menu2">
              <h3>Hourly</h3>
              <motion.div
                className="weather_mid_menu_hourly"
                drag="x"
                dragConstraints={{ right: 0, left: -width }}
              >
                {data?.hourly?.time?.map((item, index) => (
                  <motion.div
                    className="weather_mid_menu_hourly_box weather_mid_menu_daily_box"
                    key={index}
                  >
